...
contract JustHodlBase is Context, IERC20 {
    ...
    function _penalty(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: penalty from the zero address");

        _beforeTokenTransfer(account, address(0), amount);

        _balances[account] = _balances[account].sub(amount, "ERC20: penalty amount exceeds balance");
    }

    function _penaltyFrom(address account, uint256 amount) internal virtual {
        uint256 decreasedAllowance = allowance(account, _msgSender()).sub(amount, "ERC20: penalty amount exceeds allowance");
